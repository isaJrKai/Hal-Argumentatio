...
import { Network, Database, TrendingUp, Shield, RefreshCw, Brain, Focus, Fingerprint, Code2 } from 'lucide-react';
  ...
  const drawCanvas = () => {
    ...
    linksRef.current.forEach(link => {
      ...
      if (isWeak) {
        ...
        ctx.strokeStyle = \`rgba(16, 185, 129, \${Math.max(0.1, link.value / 10)})\`;
        ctx.setLineDash([]);
      }
        ...
        <div className="flex-1 bg-[#050505] relative overflow-hidden">
          <div className="absolute top-4 left-4 z-10 space-y-2 pointer-events-none">
            <div className="px-3 py-1.5 bg-black/40 backdrop-blur-md border border-white/10 rounded text-[10px] font-mono text-white/70 uppercase flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>Force-Directed Insight Topology
            </div>
          ...
          <canvas ref={canvasRef} className="w-full h-full mix-blend-screen" style={{ filter: 'contrast(1.2) brightness(1.1)' }} />
          <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-black to-transparent pointer-events-none flex items-end p-6">
            <div className="w-full">
              ...
              <div className="font-mono text-[10px] text-accent/80 space-y-1.5 h-16 overflow-hidden opacity-70">
                ...
                {(networkData.nodes || []).length === 0 && (<p>{">"} Awaiting autonomous insights from DB...</p>)}
              </div>
            </div>
          </div>
        </div>
